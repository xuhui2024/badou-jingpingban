...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        y_pred = self.linear(x)  # (batch_size, input_size, 5) -> (batch_size, 5)
        #y_pred = self.activation(x, 0)  预测值y_pred使用softmax后，loss值降不下来,而预测值y_pred直接采用linear，loss值下降很快
        if y is not None:
